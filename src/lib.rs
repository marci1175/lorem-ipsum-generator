...
const LOREM_IPSUM: [&str; 318] = [
    ...
    "magnam", "aliquam", "quaerat", "voluptatem.", "Ut", "enim", "ad", "minim", "veniam,",
...
pub fn lorem_ipsum_generator(lenght: i32) -> String {
    let mut str_vec = String::new();

    for _ in 0..lenght {
        let index = rand::thread_rng().gen_range(0..LOREM_IPSUM.len());
        
        str_vec.push_str(&format!("{} ", LOREM_IPSUM[index]));
    }

    str_vec.remove(str_vec.len() - 1);
